...
class ExpurgationState extends MusicBeatState
{
	...
	public static var staticVar:ExpurgationState;
	...
	public static var instance:ExpurgationState;
	...
	override public function create()
	{
		...
		FlxG.sound.cache(Paths.inst(ExpurgationState.SONG.song));
		FlxG.sound.cache(Paths.voices(ExpurgationState.SONG.song));
	...
	function startSong():Void
	{
		...
		if (!paused)
			FlxG.sound.playMusic(Paths.inst(ExpurgationState.SONG.song), 1, false);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
			vocals = new FlxSound().loadEmbedded(Paths.voices(ExpurgationState.SONG.song));
		else
			vocals = new FlxSound();
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && ExpurgationState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			if (curBeat % 4 == 0)
			{
				// trace(ExpurgationState.SONG.notes[Std.int(curStep / 16)].mustHitSection);
			}

			if (camFollow.x != dad.getMidpoint().x + 150 && !ExpurgationState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				camFollow.setPosition(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
			...
			if (ExpurgationState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
			...
			if (generatedMusic)
				{
					notes.forEachAlive(function(daNote:Note)
					{
						...
						// daNote.y = (strumLine.y - (songTime - daNote.strumTime) * (0.45 * ExpurgationState.SONG.speed));
	...
	public function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			else
			{
				...
				trace(ExpurgationState.storyPlaylist[0].toLowerCase() + difficulty);
				...
				ExpurgationState.SONG = Song.loadFromJson(ExpurgationState.storyPlaylist[0].toLowerCase() + difficulty, ExpurgationState.storyPlaylist[0]);
				FlxG.sound.music.stop();
				switch(song.toLowerCase())
				{
					case 'improbable-outset':
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/HankFuckingShootsTricky.webm", new ExpurgationState()));
					case 'madness':
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/HELLCLOWN_ENGADGED.webm",new ExpurgationState()));
					default:
						LoadingState.loadAndSwitchState(new ExpurgationState());
				}
			}
		}
		else
	...
	function toPlay() {FlxG.switchState(new ExpurgationState());}
